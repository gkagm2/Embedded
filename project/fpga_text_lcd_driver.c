...
#define IOM_FPGA_TEXT_LCD_ADDRESS 0x07000100 // pysical address - 32 Byte (16 * 2)
...
ssize_t iom_fpga_text_lcd_read(struct file *inode, char *gdata, size_t length, loff_t *off_what);
...
// when read from fpga_text_lcd device, call this function
ssize_t iom_fpga_text_lcd_read(struct file *inode, char *gdata, size_t length, loff_t *off_what)
{
	int i;
	unsigned char value[32];
	...
	for(i=0;i<length;i++) {
		value[i] = inb((unsigned int)iom_fpga_text_lcd_addr+i);
	}

	if (copy_to_user(gdata, value, length))
		return -EFAULT;
